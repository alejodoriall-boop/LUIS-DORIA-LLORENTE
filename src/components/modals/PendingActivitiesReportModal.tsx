...
export const PendingActivitiesReportModal: React.FC<PendingActivitiesReportModalProps> = ({
  ...
  return (
    ...
    >
      <div className="bg-[#15241C] rounded-3xl max-w-5xl w-full p-4 sm:p-6 border-2 border-white/10 shadow-2xl animate-in fade-in zoom-in-95 my-auto max-h-[92vh] flex flex-col">
        {/* MODAL HEADER */}
        <div className="flex items-start justify-between pb-4 border-b border-[#eeeeee] shrink-0 gap-3">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-[#0D1A13] text-white rounded-2xl shadow-sm">
          ...
          <button
            onClick={onClose}
            className="p-2 text-[#717973] hover:text-black rounded-xl hover:bg-[#1F3327] transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
          ...
          <button
            ...
            className={`p-3 rounded-2xl border text-left transition-all ${
              selectedTimeFilter === 'all'
                ? 'bg-[#0D1A13] text-white border-[#012d1d] shadow-md'
                : 'bg-[#0D1A13] border-white/10 text-white hover:bg-[#1F3327]'
            }`}
          >
          ...
          <button
            ...
            className={`p-3 rounded-2xl border text-left transition-all ${
              selectedTimeFilter === 'overdue'
                ...
                : overdueCount > 0
                ? 'bg-rose-950/30 border-rose-300 text-rose-950'
                : 'bg-[#0D1A13] border-white/10 text-white'
            }`}
          >
          ...
          <button
            ...
            className={`p-3 rounded-2xl border text-left transition-all ${
              selectedTimeFilter === 'today'
                ...
                : 'bg-emerald-950/30 border-emerald-200 text-emerald-950'
            }`}
          >
          ...
          <button
            ...
            className={`p-3 rounded-2xl border text-left transition-all ${
              selectedTimeFilter === 'completed'
                ...
                : 'bg-blue-950/30 border-blue-200 text-blue-950'
            }`}
          >
        ...
        {/* ACTION BAR & FILTERS */}
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2.5 mb-3 bg-[#1F3327] p-2.5 rounded-2xl border border-white/10 shrink-0">
          <div className="flex items-center gap-2 flex-1">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 text-xs font-semibold rounded-xl bg-[#15241C] border border-white/15 focus:outline-none focus:border-[#012d1d]"
              />
            </div>
            ...
            <select
              ...
              className="py-1.5 px-2 text-xs font-bold rounded-xl bg-[#15241C] border border-white/15 focus:outline-none focus:border-[#012d1d]"
            >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="px-3.5 py-2 bg-[#0D1A13] hover:bg-[#123F2A] text-white font-black rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
            >
            ...
            <button
              onClick={safePrint}
              className="px-3 py-2 bg-[#15241C] hover:bg-[#202E25] text-white font-bold rounded-xl text-xs border border-white/15 flex items-center gap-1.5 transition-all cursor-pointer"
          ...
          >
            <div className="flex items-center justify-between border-b border-emerald-200 pb-2">
              ...
              <button
                ...
                className="text-[#A5B8AC] hover:text-black font-bold"
              >
                ✕
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="sm:col-span-2">
                <label className="block font-bold text-white mb-1">Título de la Actividad *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-2 rounded-xl border border-white/15 bg-[#15241C] font-semibold focus:border-[#012d1d] focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 rounded-xl border border-white/15 bg-[#15241C] font-bold focus:border-[#012d1d] focus:outline-none"
                >
                  {Object.entries(CATEGORY_CONFIG).map(([key, cfg]) => (
                    <option key={key} value={key}>
                      {cfg.label}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              <div>
                <label className="block font-bold text-white mb-1">Nivel de Prioridad</label>
                <select
                  value={priority}
                  onChange={(e) => setPriority(e.target.value as PendingActivityPriority)}
                  className="w-full p-2 rounded-xl border border-white/15 bg-[#15241C] font-bold text-white focus:border-[#012d1d] focus:outline-none"
                >
                  <option value="alta">🚨 Urgente / Alta</option>
                  <option value="media">⚡ Media</option>
                  <option value="normal">🟢 Normal</option>
              ...
              <div>
                <label className="block font-bold text-white mb-1">Fecha Programada *</label>
                <input
                  type="date"
                  required
                  value={scheduledDate}
              ...
              <div>
                <label className="block font-bold text-white mb-1">Hora Estimada</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-white mb-1">Trabajador Responsable</label>
                {employees.length > 0 ? (
                  <select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block font-bold text-white mb-1">Lote / Animal Afectado</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-white mb-1">Potrero / Ubicación de Finca</label>
                <input
                  type="text"
            ...
            <div>
              <label className="block font-bold text-white mb-1">Instrucciones o Notas Especiales</label>
              <textarea
                rows={2}
                placeholder="Detalle dosis, insumos requeridos o procedimiento específico..."
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                className="w-full p-2 rounded-xl border border-white/15 bg-[#15241C] font-medium focus:border-[#012d1d] focus:outline-none"
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                type="button"
                onClick={() => setShowNewForm(false)}
                className="px-4 py-2 bg-[#15241C] hover:bg-[#202E25] text-white font-bold rounded-xl text-xs border border-white/15"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="px-5 py-2 bg-[#0D1A13] hover:bg-[#123F2A] text-white font-black rounded-xl text-xs flex items-center gap-1.5 shadow-md cursor-pointer"
              >
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {filteredActivities.length === 0 ? (
            <div className="p-8 text-center bg-[#0D1A13] border-2 border-dashed border-white/10 rounded-3xl my-4">
              <CheckCircle2 className="w-12 h-12 text-emerald-600 mx-auto mb-2 opacity-60" />
              <h4 className="font-bold text-white text-sm">No hay actividades pendientes en este filtro</h4>
            ...
            filteredActivities.map((act, idx) => {
              ...
              return (
                <div
                  ...
                  className={`p-3.5 sm:p-4 rounded-2xl border transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
                    act.status === 'completada'
                      ? 'bg-[#0D1A13] border-white/10 opacity-75'
                      : isOverdue
                ...
                >
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-x-4 gap-y-1 flex-wrap text-[11px] text-[#717973] font-medium pt-0.5">
                        ...
                        {act.assignedLotOrAnimal && (
                          <div className="flex items-center gap-1 font-semibold text-amber-900 bg-amber-950/30 px-1.5 py-0.5 rounded border border-amber-200">
                            <span>🐄 {act.assignedLotOrAnimal}</span>
                          </div>
                        )}

                        {act.locationPaddock && (
                          <div className="flex items-center gap-1 font-semibold text-emerald-900 bg-emerald-950/30 px-1.5 py-0.5 rounded border border-emerald-200">
                      ...
                      {act.notes && (
                        <p className="text-[11px] text-white font-medium bg-slate-100/80 p-2 rounded-xl border border-white/10 mt-1">
                  ...
                  {/* BARRAS DE ACCIÓN */}
                  <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center border-t sm:border-t-0 pt-2 sm:pt-0 border-white/10 w-full sm:w-auto justify-end flex-wrap">
                    {onNavigateToTab && (
                      <button
                        onClick={() => {
                        ...
                        className="px-2.5 py-1.5 bg-[#1F3327] hover:bg-emerald-950/30 text-white hover:text-emerald-800 font-bold rounded-xl text-xs flex items-center gap-1 transition-all cursor-pointer border border-white/10 hover:border-emerald-200"
                    ...
                    {act.status !== 'completada' ? (
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-[#202E25] hover:bg-slate-300 text-white font-bold rounded-xl text-xs transition-all cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-950/30 rounded-xl transition-colors cursor-pointer"
        ...
        {completingActivity && (
          <div className="fixed inset-0 z-60 bg-black/60 flex items-center justify-center p-4">
            <form
              ...
              className="bg-[#15241C] rounded-2xl max-w-md w-full p-5 border-2 border-emerald-300 shadow-2xl space-y-3 animate-in zoom-in-95 text-xs"
            >
              <div className="flex items-center justify-between border-b pb-2">
                ...
                <button type="button" onClick={() => setCompletingActivity(null)}>
                  <X className="w-5 h-5 text-[#A5B8AC]" />
                </button>
              </div>

              <div className="p-2.5 bg-emerald-950/30 rounded-xl border border-emerald-200 text-white font-bold">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 rounded-xl border border-white/15 font-semibold focus:outline-none focus:border-[#012d1d]"
                />
              </div>

              <div>
                <label className="block font-bold text-white mb-1">Observaciones / Novedades de Campo</label>
                <textarea
                  rows={2}
                  placeholder="Ej. Se aplicó dosis completa sin inconvenientes. Quedan 3 frascos en bodega..."
                  value={completionNotes}
                  onChange={(e) => setCompletionNotes(e.target.value)}
                  className="w-full p-2 rounded-xl border border-white/15 font-medium focus:outline-none focus:border-[#012d1d]"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-3.5 py-2 bg-[#1F3327] text-white font-bold rounded-xl"
                >
                  Cancelar
                </button>
                <button
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[#eeeeee] mt-3 shrink-0 text-xs">
          ...
          <button
            onClick={onClose}
            className="px-5 py-2.5 bg-[#0D1A13] hover:bg-[#123F2A] text-white font-bold rounded-xl transition-colors ml-auto cursor-pointer"
          >
            Cerrar Reporte
          </button>
        </div>
