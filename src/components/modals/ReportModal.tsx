...
export const ReportModal: React.FC<ReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#15241C] rounded-2xl max-w-4xl lg:max-w-5xl w-full p-6 border-2 border-white/10 shadow-2xl animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between pb-3 border-b border-[#eeeeee]">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-[#123F2A] text-white rounded-xl">
              <BarChart3 className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-white">Reporte Comercial de Ganado</h3>
        ...
        <div className="space-y-4 my-4 text-xs">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3.5 bg-[#f3f3f3] rounded-xl border border-white/10">
              <span className="text-[10px] text-[#79564b] font-bold uppercase block">
                Total Cabezas
              </span>
              <span className="font-mono text-xl font-bold text-white">{totalHeads}</span>
            </div>
          ...
          {/* Lot Breakdown Table */}
          <div>
            <h4 className="font-bold text-white mb-2">Desglose por Potrero / Lote</h4>
            <div className="border border-white/10 rounded-xl overflow-hidden">
              <table className="w-full text-left border-collapse text-xs">
                <thead className="bg-[#f3f3f3] border-b border-white/10">
                  <tr>
        ...
        <div className="flex justify-between pt-3 border-t border-[#eeeeee]">
          <button
            onClick={safePrint}
            className="px-4 py-2.5 bg-[#f3f3f3] hover:bg-[#e8e8e8] text-white font-bold rounded-xl text-xs flex items-center gap-1.5 transition-colors"
          >
