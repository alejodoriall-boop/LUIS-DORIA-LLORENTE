    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs overflow-y-auto" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-3xl border border-[#c1c8c2] card-shadow max-w-xl w-full overflow-hidden my-6 animate-in fade-in zoom-in-95 duration-150 flex flex-col">
        ...
        <div className="p-6 space-y-5 bg-[#f9f9f9]">
          {/* Dropzone */}
          <div className="border-2 border-dashed border-[#2d6a4f] rounded-2xl p-6 bg-white text-center hover:bg-[#c1ecd4]/10 transition-colors relative cursor-pointer">
            <input
              type="file"
          ...
          <div className="flex items-center justify-between p-3 bg-white rounded-2xl border border-[#c1c8c2]">
            <div className="flex items-center gap-2 text-xs">
              <Sparkles className="w-4 h-4 text-amber-500" />
              <span className="text-[#012d1d]">¿No tienes un archivo KML a mano?</span>
            </div>
            <button
              onClick={handleLoadDemo}
              className="bg-[#1b4332] hover:bg-[#2d6a4f] text-white px-3 py-1.5 rounded-xl text-xs font-bold transition-colors"
            >
              Cargar Ejemplo KML
            </button>
          </div>
          ...
          {parsedPaddocks.length > 0 && (
            <div className="bg-white p-4 rounded-2xl border border-[#c1c8c2] card-shadow space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase text-[#012d1d] flex items-center gap-1.5">
              ...
              <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1">
                {parsedPaddocks.map((p, idx) => (
                  <div
                    key={p.id}
                    className="flex items-center justify-between p-2 rounded-xl bg-[#f3f3f3] text-xs border border-[#c1c8c2]"
                  >
        ...
        {/* Footer */}
        <div className="bg-[#eeeeee] p-4 border-t border-[#c1c8c2] flex items-center justify-between">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl border border-[#c1c8c2] text-xs font-bold text-[#414844] hover:bg-white transition-colors"
          >
            Cancelar
          </button>
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-1.5 transition-all ${
              parsedPaddocks.length > 0
                ? 'bg-[#ffba38] hover:bg-[#ffdeac] text-[#523700] tactical-shadow cursor-pointer'
