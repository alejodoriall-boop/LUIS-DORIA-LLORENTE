...
export const RightNotificationSidebar: React.FC<RightNotificationSidebarProps> = ({
  ...
  return (
    <aside className="w-full md:w-80 lg:w-88 shrink-0 bg-[#F5F7F3] border-t md:border-t-0 md:border-l border-[#D6DED7] flex flex-col md:h-[calc(100vh-4rem)] md:sticky md:top-16 z-30 transition-all overflow-hidden text-[#18241D]">
      {/* Header */}
      <div className="p-3.5 md:p-4 border-b border-[#075239] flex items-center justify-between shrink-0 bg-[#043825] text-white">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-white/10 text-[#C9A35A] flex items-center justify-center border border-white/10 shrink-0">
            <Activity className="w-3.5 h-3.5" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-xs text-[#F5F2E9] leading-none truncate">Operación & Avisos</h3>
              <span className="bg-white/15 text-[#C9A35A] text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 font-mono">
                {displayActivities.length}
              </span>
            </div>
            <p className="text-[11px] text-[#B9C3BB] mt-0.5 truncate max-w-[170px]">
              {currentFarmName}
            </p>
          </div>
        </div>
        ...
        <button
          onClick={onToggle}
          className="text-[#B9C3BB] hover:text-white hover:bg-white/10 p-1.5 rounded-lg transition-colors cursor-pointer shrink-0"
          title="Ocultar panel"
        >
      ...
      {/* Content Area */}
      <div className="flex-1 overflow-y-auto p-3.5 space-y-4 max-w-full custom-scrollbar bg-[#F5F7F3] text-[#18241D]">
        {/* WhatsApp Bot Quick Access Card */}
        {onOpenWhatsAppModal && (
          <div
            onClick={onOpenWhatsAppModal}
            className="w-full bg-white hover:bg-[#EEF2ED] text-[#18241D] p-3.5 rounded-2xl shadow-xs border border-[#25D366]/40 cursor-pointer transition-all flex items-center justify-between gap-2 group"
          >
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="w-8 h-8 rounded-xl bg-[#25D366] text-white flex items-center justify-center font-bold shrink-0 shadow-xs">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-[#526158] mt-0.5 truncate">Audios, fotos y textos en tiempo real</p>
              </div>
            </div>

            <span className="text-[10px] bg-[#EEF2ED] group-hover:bg-[#DDEBE3] text-[#043825] px-2.5 py-1 rounded-lg font-bold transition-colors shrink-0">
              Probar
            </span>
          </div>
        )}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="text-[11px] font-bold text-[#0D1410] bg-[#C9A35A] hover:bg-[#B78F42] px-2.5 py-1 rounded-lg flex items-center gap-1.5 cursor-pointer transition-colors shadow-xs shrink-0"
              ...
              {showModuleMenu && (
                <div className="absolute right-0 top-7 z-50 w-64 bg-white rounded-2xl shadow-xl border border-[#D6DED7] p-2 text-xs space-y-1 animate-in fade-in zoom-in-95 text-[#18241D]">
                  ...
                  <div className="space-y-0.5 max-h-72 overflow-y-auto pt-1 custom-scrollbar">
                    <button
                      ...
                      className="w-full p-1.5 rounded-xl hover:bg-[#EEF2ED] text-left flex items-center gap-2 transition cursor-pointer group"
                    >
          ...
          <div className="bg-white border border-[#D6DED7] rounded-2xl divide-y divide-[#E1E6E1] overflow-hidden w-full shadow-2xs">
            {displayActivities.slice(0, 5).map((act, idx) => {
              ...
              return (
                <div
                  key={`${act.id}-${idx}`}
                  className="p-2.5 sm:p-3 flex items-center justify-between hover:bg-[#F5F7F3] transition-colors cursor-pointer group gap-2 w-full"
        ...
        <section className="space-y-2.5 pt-2 border-t border-[#D6DED7]">
            ...
            <div className="bg-white border border-rose-200 rounded-2xl p-3 shadow-2xs space-y-1.5 transition-all">
              ...
              <div className="pt-1 flex items-center justify-between border-t border-[#F5F7F3]">
                <button
                  ...
                  className="text-[10.5px] font-bold text-rose-700 hover:text-rose-900 flex items-center gap-1 cursor-pointer"
                >
          ...
          {(filterTab === 'all' || filterTab === 'sanitary') && (
            <div
              ...
              className="bg-white border border-amber-200 rounded-2xl p-3 shadow-2xs space-y-1.5 transition-all cursor-pointer hover:border-amber-400 group"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
          ...
          {activeMastitisCount > 0 && (filterTab === 'all' || filterTab === 'sanitary') && (
            <div
              ...
              className="bg-white border border-sky-200 rounded-2xl p-3 shadow-2xs space-y-1.5 transition-all cursor-pointer hover:border-sky-400 group"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
          ...
          {(filterTab === 'all' || filterTab === 'tasks') && (
            <div className="bg-white rounded-2xl p-3 border border-[#D6DED7] shadow-2xs space-y-2">
                  ...
                  pendingActivities.slice(0, 3).map((task) => {
                    ...
                    return (
                      <div
                        ...
                        className="flex items-start justify-between gap-2 p-2 bg-[#F5F7F3] hover:bg-[#EEF2ED] rounded-xl border border-[#D6DED7] cursor-pointer group transition-colors"
              ...
              <button
                ...
                className="w-full text-[10.5px] font-bold text-[#18241D] bg-[#EEF2ED] hover:bg-[#DDEBE3] p-1.5 rounded-xl transition-colors border border-[#D6DED7] flex items-center justify-center gap-1 cursor-pointer"
              >
                <span>Gestor Completo de Tareas ({pendingActivitiesCount})</span>
                <ChevronRight className="w-3 h-3 text-[#526158]" />
              </button>
            </div>
          )}
          ...
          {filterTab === 'all' && (
            <div
              ...
              className="bg-white hover:bg-[#F5F7F3] rounded-2xl p-3 border border-[#D6DED7] shadow-2xs space-y-1 cursor-pointer group transition-colors"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
      ...
      {/* Minimal Footer */}
      <div className="px-3 py-2 bg-[#EEF2ED] border-t border-[#D6DED7] text-center text-[10px] text-[#7A877F] font-normal shrink-0 font-mono">
